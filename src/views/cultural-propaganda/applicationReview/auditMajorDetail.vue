  ...
  <div class="beeQuantifyProgressCon" v-loading="loading">
    ...
    <div class="bottom" :class="[justForCheck ? 'setFlexStart' : '']">
      <div class="base-info boxCommon box">
        <div class="header">养殖大户<span></span></div>
        ...
        <!-- <p><span>养殖地点</span>{{ globalData.train_address }}</p> -->
        <!-- <p><span>养殖年限</span>{{ globalData.train_year }}</p> -->
        <p><span>养殖蜂箱数</span>{{ globalData.train_hive }}</p>
      </div>
      <div
...
<script>
import { beeMajoriInfo, beeMajorAudit } from "@/api/bee";
export default {
  ...
  beforeDestroy() {
    sessionStorage.removeItem("afterAnotherFlag2", "temp");
  },
  methods: {
    initCurrentData() {
      this.queryType = this.$route.query.type;
      const afterFlag = sessionStorage.getItem("afterAnotherFlag2");
      if (afterFlag) {
      ...
      beeMajoriInfo(Kabel)
        .then(res => {
          if (res.code === 0) {
            ...
            this.idList = this.globalData.bee_image_url;
            this.idList2 = this.globalData.pro_image_url;
            const statusImg = {
    ...
    submit() {
      ...
      const shaoKahn = {
        id: this.globalData.major_id,
        audit_status: this.radio === 2 ? "3" : "2",
        comment: this.content
      };
      console.log("shaoKahn", shaoKahn);
      beeMajorAudit(shaoKahn)
        .then(res => {
          if (res.code === 0) {
            this.$message.success("提交成功");
            sessionStorage.setItem("afterAnotherFlag2", "temp");
            this.initCurrentData();
            this.afterEdit = true;
          } else {
            this.$message.error(res.msg);
